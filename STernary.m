...
% >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
% ver 1.1.0 更新
        function pcolorHdl=SPcolor(obj,A,B,C,N,varargin)
            if isempty(N),N=10;end
            [X,Y]=obj.ternData(A,B,C);
            HSet=linspace(1,0,N+1).*sqrt(3)./2;
            XLSet=linspace(1/2,0,N+1);
            XRSet=linspace(1/2,1,N+1);
            for i=1:N
                Xi1=[XLSet(i),XRSet(i)];
                Xi2=[XLSet(i+1),XRSet(i+1)];
                YY=[HSet(i+1),HSet(i+1),HSet(i)];
                for j=1:i
                    XX=[Xi2(1)+(j-1).*(Xi2(2)-Xi2(1))./i, Xi2(1)+j.*(Xi2(2)-Xi2(1))./i, Xi1(1)+(j-1).*(Xi1(2)-Xi1(1))./(i-1)];
                    XX(isnan(XX))=1/2;
                    V1=[[X-XX(1),Y-YY(1)]./sqrt((X-XX(1)).^2+(Y-YY(1)).^2),...
                        [X-XX(2),Y-YY(2)]./sqrt((X-XX(2)).^2+(Y-YY(2)).^2),...
                        [X-XX(3),Y-YY(3)]./sqrt((X-XX(3)).^2+(Y-YY(3)).^2)];
                    V2=[[XX(2)-XX(1),YY(2)-YY(1)]./sqrt((XX(2)-XX(1)).^2+(YY(2)-YY(1)).^2),...
                        [XX(3)-XX(2),YY(3)-YY(2)]./sqrt((XX(3)-XX(2)).^2+(YY(3)-YY(2)).^2),...
                        [XX(1)-XX(3),YY(1)-YY(3)]./sqrt((XX(1)-XX(3)).^2+(YY(1)-YY(3)).^2)];
                    IPBool=[V1(:,1).*V2(:,1)+V1(:,2).*V2(:,2),...
                            V1(:,3).*V2(:,3)+V1(:,4).*V2(:,4),...
                            V1(:,5).*V2(:,5)+V1(:,6).*V2(:,6)];
                    IPBool(isnan(IPBool))=1;IPBool=IPBool>1/2;
                    InTriNum=sum(sum(IPBool,2)==3);
                    pcolorHdl(i,j)=fill(obj.ax,XX,YY,InTriNum,varargin{:});
                    pcolorHdl(i,j).Annotation.LegendInformation.IconDisplayStyle='off';
                end
            end
            for i=2:N
                Xi1=[XLSet(i+1),XRSet(i+1)];
                Xi2=[XLSet(i),XRSet(i)];
                YY=[HSet(i),HSet(i),HSet(i+1)];
                for j=1:(i-1)
                    XX=[Xi2(1)+(j-1).*(Xi2(2)-Xi2(1))./(i-1), Xi2(1)+j.*(Xi2(2)-Xi2(1))./(i-1),Xi1(1)+j.*(Xi1(2)-Xi1(1))./i];
                    XX(isnan(XX))=1/2;
                    V1=[[X-XX(1),Y-YY(1)]./sqrt((X-XX(1)).^2+(Y-YY(1)).^2),...
                        [X-XX(2),Y-YY(2)]./sqrt((X-XX(2)).^2+(Y-YY(2)).^2),...
                        [X-XX(3),Y-YY(3)]./sqrt((X-XX(3)).^2+(Y-YY(3)).^2)];
                    ...
                    IPBool(isnan(IPBool))=1;IPBool=IPBool>1/2;
                    InTriNum=sum(sum(IPBool,2)==3);
                    pcolorHdl(i-1+N,j)=fill(obj.ax,XX,YY,InTriNum,varargin{:});
                    pcolorHdl(i-1+N,j).Annotation.LegendInformation.IconDisplayStyle='off';
